...
saveas(gcf,strcat(pwd,'\image_result_p3\input_image_',num2str(j),'.png'))
     %%%apply preprocessing
    ...
    figure;plot(output);
     ...
     ylabel('Amplitude');
%      imwrite(figure,'image_result_p2/input_image1.png')
saveas(gcf,strcat(pwd,'\image_result_p3\preprocessing_image_',num2str(j),'.png'))
% saveas(gca, fileName);
  output=output(1:15000,:);
   ...
   figure;plot(FMatrix);
        xlabel('Number of samples');
     ylabel('Amplitude');
   saveas(gcf,strcat(pwd,'\image_result_p3\mfkcc_image_',num2str(j),'.png'))
 rolloffPoint = spectralRolloffPoint(output,fs);
f2=reshape(rolloffPoint,1,[]);
  figure;plot(f2);
      xlabel('Number of samples');
     ylabel('Amplitude');
       saveas(gcf,strcat(pwd,'\image_result_p3\spectral_rolloff_image_',num2str(j),'.png'))
  centroid = spectralCentroid(output,fs);
f3=centroid';
 figure;plot(f3);
      xlabel('Number of samples');
     ylabel('Amplitude');
      saveas(gcf,strcat(pwd,'\image_result_p3\spectral_centroid_image_',num2str(j),'.png'))
     flatness = spectralFlatness(output,fs);
 figure;plot(flatness);
      xlabel('Number of samples');
     ylabel('Amplitude');
      saveas(gcf,strcat(pwd,'\image_result_p3\spectral_flatness_image_',num2str(j),'.png'))